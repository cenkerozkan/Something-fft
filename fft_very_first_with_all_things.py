...
OUTPUT_DIR = 'balancing_analysis_results'
...
def plot_vibration_polar(axis_name, df, output_dir):
    ...
    clean_df = df.dropna(subset=[axis_name, 'Encoder'])
    if clean_df.empty:
        print(f"No valid data points for {axis_name} and Encoder after dropping NaNs. Skipping polar plot.")
        return None
    ...
    r = clean_df[axis_name].values
    colors = clean_df.index
...
def main():
    ...
    print("\n--- DataFrame Info ---")
    df.info()
    print("\n--- NaN counts per column (before any processing in main) ---")
    print(df.isnull().sum())
    ...
    print("\n--- Vibration Analysis for Balancing ---")
    for axis_name in axes_to_analyze:
        print(f"\n--- Processing Analysis for {axis_name}-axis ---")

        # Ensure data for current axis is clean for polar plot and FFT
        current_axis_signal = df[axis_name].fillna(0).values

        if len(current_axis_signal) == 0:
            print(f"{axis_name}-axis no data. Skipping.");
            continue

        # FFT Analysis
        freqs, magnitude = compute_fft(current_axis_signal, sample_rate)
        if len(freqs) > 0:
            peak_freqs, peak_mags = find_peak_frequencies(freqs, magnitude, num_peaks=num_peaks_to_find)
            # (Printing FFT peaks - code omitted for brevity, same as before)
            ...
            print(f"FFT computation failed for {axis_name}-axis.")

        # Polar Plot with Encoder & Max Vibration Angle
        print(f"\nGenerating color-coded polar plot for {axis_name}-axis...")
        # Pass the original df for plot_vibration_polar as it handles NaNs internally for the specific axis
        angle_max_vib_deg = plot_vibration_polar(axis_name, df, OUTPUT_DIR)

        if angle_max_vib_deg is not None:
            correction_angle = (angle_max_vib_deg + 180) % 360
            print(f"  Estimated 'heavy spot' for {axis_name}-axis is around: {angle_max_vib_deg:.1f}°")
            print(f"  Consider placing a trial weight for {axis_name}-axis around: {correction_angle:.1f}°")
        else:
            print(f"  Could not determine angle of maximum vibration for {axis_name}-axis.")
    ...
    print("1. The angles provided are ESTIMATES of the 'heavy spot' based on max observed vibration.")
    print(
